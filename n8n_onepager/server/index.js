...
app.post('/api/waitlist', async (req, res) => {
  try {
    ...
    const adminText = `New waitlist entry\n\n` +
      ...
      `IP: ${ip}\n` +
      `UA: ${ua}`;
    await tr.sendMail({ from: SMTP_FROM, to: WAITLIST_TO, subject: adminSubject, text: adminText, replyTo: emailStr, envelope: { from: SMTP_FROM, to: WAITLIST_TO } });
    ...
    try {
      await tr.sendMail({ from: SMTP_FROM, to: emailStr, subject: confSubject, text: confText, replyTo: WAITLIST_TO, envelope: { from: SMTP_FROM, to: emailStr } });
    } catch {}
